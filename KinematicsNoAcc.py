...
class KinematicsNoAcc:
    def __init__(self,initial_state,wheel_base,map,v_range,delta_range,min_time_steps,max_time_steps,dt=0.1) -> None:
        ...
        self.result = []
        self.forward_state_vec = []
        self.backward_state_vec = []
        self.forward_state_vec.append(self.initial_state_)
        self.backward_state_vec.append(self.initial_state_)
    ...
    def forwardPropagate(self):
        ...
        forward_result = []
        forward_result.append(shapely.points(self.forward_state_vec).tolist())
    ...
    def forwardUpdate(self):
        ...
        for i in range(self.forward_steps):
            ...
            if shapely.contains(self.map,shapely.Point(self.f_result[0],self.f_result[1])):
                ...
                while(self.f_result[2]>math.pi):self.f_result[2]-=2*math.pi
                self.forward_state_vec.append([self.f_result[0],self.f_result[1],self.f_result[2]])
                self.forward_steps=i
                return True
    ...
    def backwardPropagate(self):
        ...
        if not bu:
            return False
        backward_result = []
        backward_result.append(shapely.points(self.backward_state_vec).tolist())
    ...
    def backwardUpdate(self):
        ...
        for i in range(self.backward_steps):
            ...
            if shapely.contains(self.map,shapely.Point(self.b_result[0],self.b_result[1])):
                ...
                while(self.b_result[2]>math.pi):self.b_result[2]-=2*math.pi
                self.backward_state_vec.append([self.b_result[0],self.b_result[1],self.b_result[2]])
                self.backward_steps = i
                return True
            while(self.b_result[2]<-math.pi):self.b_result[2]+=2*math.pi
            while(self.b_result[2]>math.pi):self.b_result[2]-=2*math.pi
        self.backward_state_vec.append([self.b_result[0],self.b_result[1],self.b_result[2]])
