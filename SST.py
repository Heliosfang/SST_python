...
class Node:
    def __init__(self,id,pose,cost,path_from_parent=[],parent = None,children = [],active=True):
        self.id = id
        self.pose =pose
        self.cost = cost
        self.parent = parent
...
class SST:
    def __init__(self,start,goal,map,wheel_base,v_range,delta_range,max_running_time,min_running_time = 0,bi_tree=True,time_steps_range=[1,10]):
        ...
        buffer_map = []
        for n in map.geoms:
            m_b = shapely.Polygon(n.buffer(0.0,join_style=1))
        ...
        self.circle_radius = 0.3 #define the radius of the circle node
        ...
        while time.time()-self.start_time<self.maximum_time and time.time()-self.start_time>self.minimum_time:
    ...
    def getforwardsolutionvector(self,node_ref,node=None,tree_id=0):
        ...
        if tree_id ==0:
            ...
            if not iter_node == None:
                solution_vec_ = iter_node.path_from_parent
                solution_vec_.reverse()
            # print(solution_vec_)
                while not len(iter_node.parent.path_from_parent)==0 :
                    # print("length of the parent path: ",len(iter_node.parent.path_from_parent))

                    connected_vec = iter_node.parent.path_from_parent
                    connected_vec.reverse()
                    if len(connected_vec)==0:
                        print("fck weird zero lenght vector")
                        continue
            ...
            solution_vec_.reverse()
            next_node = node_ref
            next_fst_vec = next_node.path_from_parent
            next_fst_vec.reverse()
            solution_vec_ = solution_vec_ + next_fst_vec
            # print(node.pose)
            while not len(next_node.parent.path_from_parent)==0:
                # print("length of the parent path: ",len(iter_node.parent.path_from_parent))
                connected_vec = next_node.parent.path_from_parent
                connected_vec.reverse()
                if len(connected_vec)==0:
                    print("fck weird zero lenght vector")
                    continue
        ...
        if temp_cost+(shapely.distance(shapely.Point(forward_result[0].x,forward_result[0].y),shapely.Point(self.goal_pos.x,self.goal_pos.y))/self.v_max) < self.forward_best_cost_:
            ...
            if shapely.distance(shapely.Point(forward_result[0].x,forward_result[0].y),shapely.Point(n.center.x,n.center.y))<self.circle_radius:
                ...
                if temp_cost<n_obj.cost:
                    ...
                    n_obj.active = False
                    Pnodei = Node(self.forward_snode_id,forward_result[0],temp_cost,forward_model_result[0],parent_node,[])
                ...
                Pnodei = Node(self.forward_cnode_id,forward_result[0],temp_cost,forward_model_result[0],parent_node,[])
                # print("forward_node_new path:", forward_model_result[0])
                self.f_treeVector.append(Pnodei)
        ...
        if self.bi_tree:
            ...
            if dis_to_goal <=0.3:
                ...
                if temp_best_cost < self.forward_best_cost_:
                    ...
                    self.fsolutionvector = self.getforwardsolutionvector(node_ref = self.forward_last_node)
        ...
        if temp_cost+(shapely.distance(shapely.Point(backward_result[0].x,backward_result[0].y),shapely.Point(self.start_pos.x,self.start_pos.y))/self.v_max) < self.backward_best_cost_:
            ...
            if shapely.distance(shapely.Point(backward_result[0].x,backward_result[0].y),shapely.Point(n.center.x,n.center.y))<self.circle_radius:
                ...
                if temp_cost<n_obj.cost:
                    ...
                    # update the current circle node
                    n_obj.active = False
                    Pnodei = Node(self.backward_snode_id,backward_result[0],temp_cost,backward_model_result[0],parent_node,[])
                ...
                Pnodei = Node(self.backward_cnode_id,backward_result[0],temp_cost,backward_model_result[0],parent_node,[])
                # print("forward_node_new path:", backward_model_result[0])
